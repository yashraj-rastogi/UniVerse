...
import type { Request } from "@/lib/firebase/jugaadbank"
...
export function RequestCard({ request, onContact }: RequestCardProps) {
  ...
  return (
    <Card className="h-full flex flex-col hover:shadow-lg transition-shadow border-dashed">
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="bg-secondary/10 p-2 rounded-lg shrink-0">{icon}</div>
            <CardTitle className="text-lg line-clamp-2">{request.itemName}</CardTitle>
          </div>
          <Badge variant="outline" className="shrink-0">
            {request.offeringPrice} pts
          </Badge>
        </div>
        <CardDescription className="text-xs">Requested by Roll #{request.requesterRollNumber}</CardDescription>
      </CardHeader>
      <CardContent className="flex-1 flex flex-col">
        ...
        <div className="mt-auto space-y-2">
          ...
          {onContact && (
            <Button variant="secondary" size="sm" className="w-full" onClick={() => onContact(request)}>
              Contact Requester
            </Button>
          )}
        </div>
